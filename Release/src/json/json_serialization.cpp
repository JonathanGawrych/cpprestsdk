...
void web::json::details::_Number::format(utility::string_t& stream) const
{
    if(m_number.m_type != number::type::double_type)
    {
        ...
        const auto numChars = wcsnlen_s(tempBuffer, tempSize);
#else
#ifdef _UTF16_STRINGS
        char tempUtf8Buffer[tempSize];
        int numChars;
        if (m_number.m_type == number::type::signed_type)
            numChars = snprintf(tempUtf8Buffer, tempSize, "%" PRId64, m_number.m_intval);
        else
            numChars = snprintf(tempUtf8Buffer, tempSize, "%" PRIu64, m_number.m_uintval);
        utility::conversions::to_utf16string(utf8string(tempUtf8Buffer, numChars)).copy(tempBuffer, tempSize);
#else
        int numChars;
        if (m_number.m_type == number::type::signed_type)
    ...
    else
    {
        ...
        char_t tempBuffer[tempSize];
#ifdef _WIN32
        const auto numChars = _swprintf_s_l(
            tempBuffer,
            tempSize,
            U("%.*g"),
...
#ifdef _UTF16_STRINGS
        char tempUtf8Buffer[tempSize];
        const auto numChars = snprintf(tempUtf8Buffer, tempSize, "%.*g", std::numeric_limits<double>::digits10 + 2, m_number.m_value);
        utility::conversions::to_utf16string(utf8string(tempUtf8Buffer, numChars)).copy(tempBuffer, tempSize);
#else
